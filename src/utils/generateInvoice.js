...
function drawLine(doc, y) {
  doc.strokeColor("#d9e6e3").lineWidth(1).moveTo(50, y).lineTo(545, y).stroke();
}

function drawTableRow(doc, y, columns, options = {}) {
  const { fontSize = 10.5, header = false, color = "#0f172a", background = null } = options;
...
export function generateInvoicePdf(order, res) {
  ...
  doc.font("Helvetica-Bold").fontSize(24).fillColor("#0f766e").text("MUSCLEKART", 50, 50);
  ...
  doc.font("Helvetica-Bold").fontSize(11).fillColor("#0f172a").text("Invoice Details", 50, 122);
  ...
  doc
    ...
    .text(`Order Date: ${new Date(order.order_date).toLocaleString("en-IN")}`, 50, 162)
    .text(`Payment Method: ${String(order.payment_method || "").toUpperCase()}`, 50, 179)
    .text(`Payment Status: ${String(order.payment_status || "").toUpperCase()}`, 50, 196);
  ...
  doc.font("Helvetica-Bold").fontSize(11).fillColor("#0f172a").text("Order Summary", 340, summaryY);
  ...
  doc.text("Discount", 360, summaryY).text(formatCurrency(order.discount), 455, summaryY, {
    width: 80,
    align: "right"
  });
